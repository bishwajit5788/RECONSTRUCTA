...
class AssetMetadataModel(BaseModel):
    ...
    expires_at: float  # Absolute timestamp for 2-hour retention
...
class AuditEventModel(BaseModel):
    event_id: str
    session_id: str
    action: str  # e.g. "upload", "inpaint", "export", "purge"
    timestamp: float = Field(default_factory=time.time)
    expires_at: float
...
class InMemoryCollection:
    """Mock async collection for local test runs and offline development."""
    ...
    async def update_one(self, filter_query: Dict[str, Any], update_doc: Dict[str, Any], upsert: bool = False) -> Any:
        ...
        if upsert:
            ...
            class ResUpsert:
                modified_count = 0
                upserted_id = insert_res.inserted_id
            return ResUpsert()
        class Res0:
            modified_count = 0
            upserted_id = None
...
class DatabaseManager:
    ...
    async def _setup_ttl_indexes(self):
        """Create TTL indexes on `expires_at` across temporary collections."""
        ...
        for col_name in ttl_collections:
            try:
                ...
                # expireAfterSeconds=0 causes MongoDB to remove documents when current_time >= expires_at
