"""Production backend regression tests compatible with project ownership enforcement."""
...
def create_project(project_id):
    response = client.post(
        "/api/projects",
        json={"project_id": project_id, "owner_session": OWNER, "name": "Test Project"},
        headers=HEADERS,
    )
...
def test_inpaint_with_quality_scoring():
    response = client.post("/api/inpaint", json={
        "image_data": create_sample_image_base64(120, 80),
        "bounds": {"x": 20, "y": 20, "width": 40, "height": 20},
        "algorithm": "telea", "inpaint_radius": 3,
    })
    assert response.status_code == 200
    data = response.json()
...
def test_inpaint_invalid_bounds():
    response = client.post("/api/inpaint", json={
        "image_data": create_sample_image_base64(50, 50),
...
def test_r2_file_upload_download_delete_lifecycle():
    ...
    # The authorization contract intentionally returns 403 for an unknown asset
    # so object existence cannot be enumerated by unauthenticated callers.
...
def test_mongodb_project_metadata_and_touch():
    ...
    response = client.post("/api/projects", headers=HEADERS, json={
        "project_id": project_id, "owner_session": OWNER,
        "name": "Luxury Test Project", "platform": "generic",
        "schema_version": 1, "canvas_width": 1080, "canvas_height": 1920, "node_count": 12,
    })
    assert response.status_code == 200
...
def test_two_hour_retention_purge():
    ...
    meta = storage_manager.upload_file(
        data=b"EXPIRED_DATA", project_id=project_id, purpose="working",
        content_type="text/plain", ttl_seconds=-10,
    )
...
def test_magic_bytes_rejection():
    ...
    response = client.post(
        "/api/files/upload", headers=HEADERS,
        files={"file": ("malicious.exe", io.BytesIO(b"\x00\x01\x02\x03CORRUPTED"), "application/octet-stream")},
        data={"project_id": project_id, "purpose": "uploads"},
    )
    assert response.status_code == 400
...
def test_zip_bomb_detection():
    ...
    response = client.post(
        "/api/files/upload", headers=HEADERS,
        files={"file": ("malicious.docx", io.BytesIO(buf.getvalue()), "application/vnd.openxmlformats-officedocument.wordprocessingml.document")},
        data={"project_id": project_id, "purpose": "uploads"},
    )
    assert response.status_code == 400
